...
pub async fn run_file(
    ...
    let registry_source = match registry_path {
        Some(p) => {
            if !p.is_dir() {
                return Err(anyhow!(
...
async fn run_calldata(
    ...
    registry_source: Option<&StaticSource>,
) -> CaseResult {
    ...
    // With `--registry`, resolve nested callees against the whole registry tree;
    // otherwise fall back to the descriptors sitting beside the test file.
    let local_source;
    let source: &StaticSource = match registry_source {
        Some(s) => s,
        None => {
            local_source = build_calldata_source_from_dir(
                test_path,
                descriptor,
                descriptor_path,
                decoded.chain_id,
...
fn build_calldata_source_from_dir(
    ...
    for path in paths {
        ...
        index_descriptor_file(&mut source, &mut seen, &path);
    }
    source
}
...
/// Build a calldata descriptor source by recursively indexing every contract
/// descriptor under a registry tree, so nested `calldata` sub-calls resolve to
/// callees anywhere in the registry — not just siblings of the test file. Files
/// are visited in sorted order, so resolution is deterministic; the first
/// descriptor to claim a `(chain_id, address)` wins.
fn build_calldata_source_from_registry(root: &Path) -> StaticSource {
    let mut source = StaticSource::new();
    let mut seen: HashSet<(u64, String)> = HashSet::new();
...
/// Recursively collect `*.json` descriptor files under `dir` (excluding
/// `*.tests.json`), visiting entries in sorted order at each level.
