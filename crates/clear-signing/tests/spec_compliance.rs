...
/// A salt-based EIP-712 domain (no `chainId`, replay-protected by `salt`, e.g.
/// Polygon meta-transactions) must still clear-sign when the descriptor declares
/// a domain binding the message satisfies — selecting by verifyingContract. The
/// bare verifyingContract-only case (no declared domain) keeps falling back; see
/// `test_eip712_missing_chain_id_rejected_with_descriptors`.
#[tokio::test]
async fn test_eip712_salt_domain_without_chain_id_selects_descriptor() {
    let descriptor = Descriptor::from_json(
        r#"{
            "context": { "eip712": {
                "deployments": [{"chainId": 137, "address": "0x7f19564c35c681099c0c857a7141836cf7edaa53"}],
                "domain": { "name": "Rarible" }
            } },
            "metadata": { "owner": "Rarible ExchangeV2", "enums": {}, "constants": {}, "maps": {} },
            "display": {
                "definitions": {},
                "formats": {
    ...
    let result = format_typed_data(
        &wrap_rd(
            descriptor,
            137,
            "0x7f19564c35c681099c0c857a7141836cf7edaa53",
        ),
        &typed_data,
        &EmptyDataProvider,
    )
    ...
    assert_eq!(
        result.fallback_reason(),
        None,
        "should clear-sign, not fall back"
    );
    assert_eq!(result.intent, "Meta Transaction");
    assert_eq!(result.owner.as_deref(), Some("Rarible ExchangeV2"));
    assert_eq!(
        ...
        vec![
            ...
            ("Meta Transaction Nonce".to_string(), "42".to_string()),
        ]
    );
}
