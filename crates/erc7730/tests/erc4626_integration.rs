...
fn erc4626_token_source() -> CompositeDataProvider {
    ...
    // LeadBlock USDC RWA
    custom.insert(1, "0x4ca0e178c94f039d7f202e09d8d1a655ed3fb6b6", TokenMeta {
        symbol: "USDC RWA".to_string(), decimals: 18, name: "LeadBlock USDC RWA".to_string(),
    });
    // USDC (underlying for LeadBlock)
    custom.insert(1, "0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48", TokenMeta {
        symbol: "USDC".to_string(), decimals: 6, name: "USD Coin".to_string(),
    });
    // Yield.xyz USDe
    custom.insert(1, "0x4c9edd5852cd905f086c759e8383e09bff1e68b3", TokenMeta {
        symbol: "USDe".to_string(), decimals: 18, name: "Ethena USDe".to_string(),
    });
    // Yield.xyz stk-USDe
    custom.insert(1, "0x2d152fb171353e70e45322d32bc748f8a61d9971", TokenMeta {
        symbol: "stk-USDe".to_string(), decimals: 18, name: "StakeKit Ethena USDe Vault".to_string(),
    });
...
async fn leadblock_deposit() {
    ...
    let tx = TransactionContext {
        chain_id: 1, to: LEADBLOCK_ADDR, calldata: &calldata, value: None,
    ...
    assert_eq!(get_entry_value(&result, "Send shares to"), "0x4a9fFDBc1c8d6aAbAfce2b6AFED90E714487ea74");
...
async fn leadblock_mint() {
    ...
    assert_eq!(get_entry_value(&result, "Minted shares"), "0.000000001 USDC RWA");
    assert_eq!(get_entry_value(&result, "Mint shares to"), "0x000000000000000000000000000000000000dEaD");
    assert!(result.warnings.is_empty());
}
...
async fn leadblock_withdraw() {
    ...
    assert_eq!(get_entry_value(&result, "Withdraw exactly"), "50.020219 USDC");
    assert_eq!(get_entry_value(&result, "To"), "0x8379bD16381620914d8fA3d535F6Ca9eF23ece53");
    assert_eq!(get_entry_value(&result, "Owner"), "0x8379bD16381620914d8fA3d535F6Ca9eF23ece53");
    assert!(result.warnings.is_empty());
}
...
async fn leadblock_redeem() {
    ...
    assert_eq!(get_entry_value(&result, "Shares to redeem"), "9.8314536789615253 USDC RWA");
    assert_eq!(get_entry_value(&result, "To"), "0xe0A68A5650C72f2d65B8B5de386234420B90Afed");
    assert!(result.warnings.is_empty());
}
...
async fn yieldxyz_deposit() {
    ...
    let provider = erc4626_token_source();
    let tx = TransactionContext {
        chain_id: 1, to: YIELDXYZ_ADDR, calldata: &calldata, value: None,
...
async fn yieldxyz_redeem() {
    ...
    assert_eq!(result.intent, "Redeem");
    assert_eq!(result.owner.as_deref(), Some("Yield.xyz"));
    assert_eq!(get_entry_value(&result, "Shares to redeem"), "7925772897.557314225765707281 stk-USDe");
    assert_eq!(get_entry_value(&result, "To"), "0x36272F3569DaF11Bb4464354cC136Acd9ecA1D2B");
    assert!(result.warnings.is_empty());
}
