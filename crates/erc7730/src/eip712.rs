...
fn compute_domain_separator(data: &TypedData) -> Result<[u8; 32], Error> {
    ...
    hash_typed_struct("EIP712Domain", &data.domain.to_value(), &data.types)
}
...
fn hash_typed_struct(
    ...
    for field in fields {
        ...
        let encoded_field =
            encode_typed_value(&field.field_type, field_value, types, Some(&field.name))?;
...
fn encode_typed_value(
    ...
    if let Some(element_type) = array_element_type(field_type) {
        ...
        for item in items {
            let encoded = encode_typed_value(element_type, item, types, field_name)?;
